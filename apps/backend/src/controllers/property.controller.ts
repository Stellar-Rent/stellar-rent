 ...
 */
export async function verifyPropertyController(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!validation.success) {
      res
        .status(400)
        .json(formatErrorResponse('Invalid request parameters', validation.error.errors));
      return;
    }
    ...
    res.status(200).json(formatSuccessResponse(result.data, 'Property verification completed'));
  } catch (error) {
