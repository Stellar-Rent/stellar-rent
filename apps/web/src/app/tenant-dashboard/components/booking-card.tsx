...
import { MapPin, Calendar, Star, Eye, CreditCard, Clock } from 'lucide-react';
...
const BookingCard: React.FC<BookingCardProps> = ({ booking, onViewDetails, onCancel }) => {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'ongoing': return <CreditCard className="w-3 h-3 mr-1" />;
      case 'completed': return <Star className="w-3 h-3 mr-1" />;
      case 'cancelled': return <div className="w-3 h-3 mr-1 rounded-full bg-red-500" />;
      default: return null;
    }
  };
          ...
          <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
            ...
            {isUpcoming() && booking.status === 'upcoming' && (
              <span className="text-blue-600 dark:text-blue-400 font-medium">
                {Math.ceil((new Date(booking.checkIn).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24))} days to go
              </span>
            )}
          </div>
        </div>
