...
import { AlertCircle, ArrowRight, BookOpen, Calendar, Download, Home, Share2 } from 'lucide-react';
...
export function NavigationActions() {
  ...
  const contextualNavItems = getContextualNavigation(pathname, isAuthenticated);

  const handleMyBookings = () => {
    setNavigationError(null);

    if (!isAuthenticated) {
      setNavigationError('Please log in to view your bookings');
      router.push('/login?redirect=/dashboard/tenant-dashboard');
      return;
    }
        ...
        <div className="px-6 pb-6 space-y-4">
          ...
          <div className="space-y-3">
            <Button
              ...
              disabled={isLoading}
              className="w-full bg-primary hover:bg-primary/90 text-white flex items-center justify-between group disabled:opacity-50"
              size="lg"
            >
          ...
          {contextualNavItems.length > 0 && (
            <div className="border-t pt-4">
              ...
              <div className="grid grid-cols-1 gap-2">
                {contextualNavItems.slice(0, 4).map((item) => (
                  <Button
                    key={item.label}
                    variant="outline"
                    size="sm"
